...
## Tasks / Subtasks
- [ ] Task 1: Note Tags 테이블 스키마 추가 (AC: 3)
  - [ ] `drizzle/schema.ts`에 note_tags 테이블 정의
  - [ ] note_id (FK), tag 필드 추가
  - [ ] 복합 unique 제약 조건 (note_id, tag)
  - [ ] 인덱스 추가 (note_id, tag)
  - [ ] 타입 추출 (NoteTag, NewNoteTag)
  - [ ] 마이그레이션 생성 및 적용

- [ ] Task 2: 태그 생성 프롬프트 함수 구현 (AC: 2, 4, 5)
  - [ ] `/lib/ai/prompts.ts`에 `generateTagsPrompt` 추가
  - [ ] 최대 6개 태그 생성 지시
  - [ ] 한 단어 태그 형식 지정
  - [ ] 쉼표로 구분된 형식 요청
  - [ ] 태그 정규화 함수 (`normalizeTag`) 추가

- [ ] Task 3: 태그 생성 Server Action 구현 (AC: 1, 2, 3, 4, 5, 9)
  - [ ] `/app/actions/ai.ts`에 `generateNoteTags` 추가
  - [ ] 노트 내용 토큰 제한 확인 및 자르기
  - [ ] Gemini API 호출하여 태그 생성
  - [ ] 태그 파싱 및 정규화
  - [ ] 중복 제거 및 최대 6개 제한
  - [ ] note_tags 테이블에 저장
  - [ ] 사용자 인증 및 권한 확인

- [ ] Task 4: 태그 조회 함수 구현 (AC: 10)
  - [ ] `/app/actions/ai.ts`에 `getNoteTags` 추가
  - [ ] note_id로 태그 목록 조회
  - [ ] 사용자 권한 확인
  - [ ] 태그 배열 반환

- [ ] Task 5: 태그 삭제 함수 구현 (AC: 10)
  - [ ] `/app/actions/ai.ts`에 `deleteNoteTag` 추가
  - [ ] 특정 태그 삭제 기능
  - [ ] 사용자 권한 확인

- [ ] Task 6: 노트 상세 페이지에 태그 표시 (AC: 6, 7, 8)
...
- [ ] Task 7: 태그 자동 생성 옵션 추가 (AC: 1, 10)
  - [ ] 노트 저장 시 "자동 태그 생성" 체크박스 추가
  - [ ] 체크박스 상태에 따라 태그 생성 트리거
  - [ ] localStorage에 사용자 설정 저장

- [ ] Task 8: 테스트 작성
  - [ ] 태그 프롬프트 함수 테스트
  - [ ] 태그 정규화 함수 테스트
  - [ ] Server Action 테스트 (모킹)
  - [ ] 중복 제거 테스트
  - [ ] UI 통합 테스트
